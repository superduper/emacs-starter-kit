...
1. *ERC* — Internet Relay Chat для Emacs
...
3. *Info-mode.* В директории =info= хранятся документы в
...
4. *Org-mode.* Проект «Bullshit mode on» — мой блог.
...
5. *Typopunct-mode.* Базовая российская типографика в Emacs: кавычки
   ёлочки и лапки; короткое и длиное тире.

   #+begin_src emacs-lisp :noweb yes :export none :tangle yes
   (require 'typopunct)

   (setq-default typopunct-buffer-language 'russian)
   #+end_src

6. Включает typopunct mode.
...
7. Здесь я включаю типографику в режимах org-mode и markdown.
...
8. *Magit-mode.* Режим для работы с системой контроля версий git.
