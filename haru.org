...
4. *Smex* это надстройка над режимом ido для улучшения работы с
   вызовом команд по *M-x*. После нажатия M-x доступные команды
   показываются в буфере автодополнения ido, сперва идут 7 последних
   выполненных команд, затем — команды, отсортированные по частоте
   использования, длине и в алфавитном порядке. Можно использовать
   сокращения типа «lnmd» для вызова «line-number-mode» и клавиши
   C-s и С-r для выбора следующей и предыдущей команды. 
...
6. *F11* включает полноэкранный режим. Требует наличия программы
   *wmctrl*.
...
7. *ERC* — Internet Relay Chat для Emacs.
...
9. *Info-mode.* В директории =info= хранятся документы в
...
10. *Org-mode.* Проект «Bullshit mode on» — мой блог.
...
11. Поддержка интерактивного литературного программирования в Babel —
...
12. *Typopunct-mode.* Если не вдаваться в подробности профессиональной
...
13. Загружаем /typopunct.el/ и выбираем русскую типографику:
...
14. Функция включает /typopunct-mode/.
...
15. Здесь я включаю типографику в режимах /org-mode/ и
...
16. Пакет *htmlize* служит для экспортирования текста буферов и файлов
...
17. Функция =swap-meta-super= меняет местами клавиши Meta и
...
18. SLIME, идущий в поставке ELPA урезан настолько, что годен только
...
19. *Фортунки.* Их задача проста — заменить стандартный стартовый текст на что-нибудь
...
20. *Идентация в Java* в 3 пробела.
